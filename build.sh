...
buildDockerImage () {
    ...
    echo -e "\nPreparing files"
    mkdir -p ${BUILD_DIR}
    cp -v  ${SCRIPT_DIR}/Dockerfile ${BUILD_DIR}
...
packHelmChart() {
    echo -e "\nPacking Helm chart"
    sed -E  "s|tag:.*|tag: ${DOCKER_TAG}|g" ${SCRIPT_DIR}/helm/values.yaml.orig >${SCRIPT_DIR}/helm/values.yaml
    [ -d ${BUILD_DIR}/helm_output ] && rm -rf ${BUILD_DIR}/helm_output
    mkdir -p ${BUILD_DIR}/helm_output

    helm package -d ${BUILD_DIR}/helm_output ${SCRIPT_DIR}/helm/ || errorExit "Packing helm chart ${SCRIPT_DIR}/helm failed"
}
...
pushHelmChart() {
    ...
    local chart_name=$(ls -1 ${BUILD_DIR}/helm_output/*.tgz 2> /dev/null)
    echo "Helm chart: ${chart_name}"

    [ ! -z "${chart_name}" ] || errorExit "Did not find the helm chart to deploy"
    curl --data-binary "@${chart_name}" http://${HELM_REPO}/api/charts || errorExit "Uploading helm chart failed"
    echo
}
